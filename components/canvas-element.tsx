...
export const CanvasElementComponent = memo(function CanvasElementComponent({
  ...
  useEffect(() => {
    if (element.type === "text" && element.fontFamily && typeof window !== "undefined") {
      const loadFont = (window as any).loadGoogleFont
      if (loadFont) {
        loadFont(element.fontFamily)
      }
    }
  }, [element.fontFamily, element.type])
    ...
    >
      ...
      {element.type === "text" && (
        <div
          ...
          className="w-full h-full flex items-center rounded-lg px-2 select-none outline-none"
          style={{
